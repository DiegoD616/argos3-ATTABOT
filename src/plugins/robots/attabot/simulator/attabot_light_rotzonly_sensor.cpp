/**
 * @file <argos3/plugins/robots/attabot/simulator/attabot_light_rotzonly_sensor.cpp>
 *
...
#include "attabot_light_rotzonly_sensor.h"
...
namespace argos {
   ...
   CAttabotLightRotZOnlySensor::CAttabotLightRotZOnlySensor() :
   ...
   void CAttabotLightRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the Attabot light default sensor", ex);
      }
   }
   ...
   void CAttabotLightRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
   ...
   void CAttabotLightRotZOnlySensor::Update() {
      /* Erase readings */
      for(size_t i = 0; i < m_tReadings.size(); ++i) {
         m_tReadings[i].Value = 0.0f;
      }
      /* Get Attabot orientation */
      ...
      /* Buffer for the angle of the light wrt to the Attabot */
      ...
      for(CSpace::TMapPerType::iterator it = mapLights.begin();
         ...
         if(cLight.GetIntensity() > 0.0f) {
            ...
            /* Check occlusion between the Attabot and the light */
            if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
               ...
               /* Get the distance between the light and the Attabot */
               ...
               /* Get the angle wrt to Attabot rotation */
               cAngleLightWrtRobot = cRobotToLight.GetZAngle();
               cAngleLightWrtRobot -= cOrientationZ;
               /*
                * Find closest sensor index to point at which ray hits attabot body
                * Division says how many sensor spacings there are between first sensor and point at which ray hits attabot body
   ...
   void CAttabotLightRotZOnlySensor::Reset() {
   ...
   REGISTER_SENSOR(CAttabotLightRotZOnlySensor,
                   "attabot_light", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The Attabot light sensor (optimized for 2D).",
                   ...
                   "controllers, you must include the ci_attabot_light_sensor.h header.\n\n"
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <attabot_light implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
