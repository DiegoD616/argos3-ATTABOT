/**
 * @file <argos3/plugins/robots/attabot/simulator/attabot_lidar_default_sensor.cpp>
 *
...
#include "attabot_lidar_default_sensor.h"
...
namespace argos {
   ...
   CAttabotLIDARDefaultSensor::CAttabotLIDARDefaultSensor() :
      m_pnReadings(NULL),
      m_unNumReadings(682),
   ...
   void CAttabotLIDARDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the attabot LIDAR default sensor", ex);
      }
   }
   ...
   void CAttabotLIDARDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_AttabotLIDARSensor::Init(t_tree);
         /* How many readings? */
         GetNodeAttributeOrDefault(t_tree, "num_readings", m_unNumReadings, m_unNumReadings);
         m_pcProximityEntity->AddSensorFan(
            CVector3(0.0, 0.0, ATTABOT_LIDAR_ELEVATION),
            ATTABOT_LIDAR_SENSORS_FAN_RADIUS + ATTABOT_LIDAR_SENSORS_RING_RANGE.GetMin(),
            -ATTABOT_LIDAR_ANGLE_SPAN * 0.5,
            ATTABOT_LIDAR_ANGLE_SPAN * 0.5,
            ATTABOT_LIDAR_SENSORS_FAN_RADIUS + ATTABOT_LIDAR_SENSORS_RING_RANGE.GetMax(),
            m_unNumReadings,
   ...
   void CAttabotLIDARDefaultSensor::Update() {
   ...
   void CAttabotLIDARDefaultSensor::Reset() {
   ...
   void CAttabotLIDARDefaultSensor::Destroy() {
      delete[] m_pnReadings;
   }
   ...
   long CAttabotLIDARDefaultSensor::GetReading(UInt32 un_idx) const {
   ...
   size_t CAttabotLIDARDefaultSensor::GetNumReadings() const {
   ...
   void CAttabotLIDARDefaultSensor::PowerOn() {
      m_unPowerLaserState = m_unPowerLaserState | 0x1;
      m_pcProximityEntity->SetEnabled(m_unPowerLaserState == ATTABOT_POWERON_LASERON);
   }

   /****************************************/
   /****************************************/

   void CAttabotLIDARDefaultSensor::PowerOff() {
   ...
   void CAttabotLIDARDefaultSensor::LaserOn() {
   ...
   void CAttabotLIDARDefaultSensor::LaserOff() {
   ...
   REGISTER_SENSOR(CAttabotLIDARDefaultSensor,
                   "attabot_lidar", "default",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The attabot LIDAR sensor.",
                   "This sensor accesses the attabot LIDAR sensor. The sensors return the\n"
