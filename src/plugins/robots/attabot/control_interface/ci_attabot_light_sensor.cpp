/**
 * @file <argos3/plugins/robots/attabot/control_interface/ci_attabot_light_sensor.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "ci_attabot_light_sensor.h"
...
namespace argos {
   ...
   CCI_AttabotLightSensor::CCI_AttabotLightSensor() :
      m_tReadings(8) {
   ...
   const CCI_AttabotLightSensor::TReadings& CCI_AttabotLightSensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_AttabotLightSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_AttabotLightSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_AttabotLightSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_AttabotLightSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
