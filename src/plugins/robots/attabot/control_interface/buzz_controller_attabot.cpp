#include "buzz_controller_attabot.h"
#include <argos3/core/utility/logging/argos_log.h>

/****************************************/
/****************************************/

CBuzzControllerAttabot::SWheelTurningParams::SWheelTurningParams() :
...
void CBuzzControllerAttabot::SWheelTurningParams::Init(TConfigurationNode& t_node) {
   try {
      TurningMechanism = NO_TURN;
      CDegrees cAngle;
...
static int BuzzGoToC(buzzvm_t vm) {
   ...
   buzzvm_gload(vm);
   /* Call function */
   reinterpret_cast<CBuzzControllerAttabot*>(buzzvm_stack_at(vm, 1)->u.value)->SetWheelSpeedsFromVector(cDir);
...
int BuzzSetWheels(buzzvm_t vm) {
   ...
   buzzvm_gload(vm);
   /* Call function */
   reinterpret_cast<CBuzzControllerAttabot*>(
...
int BuzzSetLEDs(buzzvm_t vm) {
   ...
   buzzvm_gload(vm);
   /* Call function */
   reinterpret_cast<CBuzzControllerAttabot*>(buzzvm_stack_at(vm, 1)->u.value)->SetLEDs(cColor);
   return buzzvm_ret0(vm);
}
...
CBuzzControllerAttabot::CBuzzControllerAttabot() :
   m_pcWheels(NULL),
   m_pcLEDs(NULL),
   m_pcGround(NULL),
   m_pcProximity(NULL),
...
void CBuzzControllerAttabot::Init(TConfigurationNode& t_node) {
   try {
      ...
      try {
         m_pcGround = GetSensor<CCI_AttabotGroundSensor>("attabot_ground");
      }
      catch(CARGoSException& ex) {}
      try {
         m_pcProximity = GetSensor<CCI_AttabotProximitySensor>("attabot_proximity");
      }
      catch(CARGoSException& ex) {}
      try {
         m_pcLight = GetSensor<CCI_AttabotLightSensor>("attabot_light");
      }
      catch(CARGoSException& ex) {}
      try {
         m_pcUltrasound = GetSensor<CCI_AttabotUltrasoundSensor>("attabot_ultrasound");
      }
      catch(CARGoSException& ex) {}
      try {
         m_pcLIDAR = GetSensor<CCI_AttabotLIDARSensor>("attabot_lidar");
      }
      catch(CARGoSException& ex) {}
      /* Initialize the rest */
...
void CBuzzControllerAttabot::UpdateSensors() {
   /*
   ...
   if(m_pcGround != NULL) {
      ...
      /* Get ground readings */
      const CCI_AttabotGroundSensor::TReadings& tGrndReads = m_pcGround->GetReadings();
   ...
   if(m_pcProximity != NULL) {
      ...
      const CCI_AttabotProximitySensor::TReadings& tProxReads = m_pcProximity->GetReadings();
   ...
   if(m_pcLight != NULL) {
      ...
      buzzvm_gstore(m_tBuzzVM);
      /* Get proximity readings */
      const CCI_AttabotLightSensor::TReadings& tLightReads = m_pcLight->GetReadings();
      /* Fill into the proximity table */
   ...
   if(m_pcUltrasound != NULL) {
      ...
      const CCI_AttabotUltrasoundSensor::TReadings& tUSReads = m_pcUltrasound->GetReadings();
...
void CBuzzControllerAttabot::SetWheelSpeedsFromVector(const CVector2& c_heading) {
...
void CBuzzControllerAttabot::SetWheels(Real f_left_speed,
                                         Real f_right_speed) {
...
void CBuzzControllerAttabot::SetLEDs(const CColor& c_color) {
...
buzzvm_state CBuzzControllerAttabot::RegisterFunctions() {
...
REGISTER_CONTROLLER(CBuzzControllerAttabot, "buzz_controller_attabot");

#ifdef ARGOS_DYNAMIC_LIBRARY_LOADING
#include <argos3/plugins/robots/attabot/simulator/attabot_entity.h>
REGISTER_BUZZ_ROBOT(CAttabotEntity);
#endif
